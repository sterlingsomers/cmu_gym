    ...
    def display_click_image(clickData,
        ...
        if dataset in IMAGE_DATASETS and clickData:
            ...
            try:
                ...
                print(error, "\nThe dataset was not found. Please generate it using generate_demo_embeddings.py")
                return
            # (MINE) Logic: click on a point and retrieve its x,y,z values. Search in the file loaded with tsne data for that particular row. Then take the index of that row and retrieve image from the image dataset
