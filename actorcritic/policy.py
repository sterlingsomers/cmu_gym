...
class FullyConvPolicy:
    ...
    def build(self):
        ...
        self.fc1 = layers.fully_connected(
            map_output_flat,
            num_outputs=256,#,512 for pacman
            activation_fn=tf.nn.relu,
            scope="fc1",
            trainable=self.trainable
        )
...
class FullyConv3DPolicy:
    ...
    def build(self):
        ...
        # non-available actions get log(1e-10) value but that's ok because it's never used
        action_id_log_probs = logclip(action_id_probs) # This one might not be necessary!!! RECHECK THIS ONE cauz now the neg entropy in TB seems positive
