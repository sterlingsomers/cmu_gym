...
class A2CAgent:
    def __init__(self, model):
        ...
        self.model = model
        self.model.compile(
            optimizer=ko.RMSprop(lr=0.0007), # Maybe use Adam?
    ...
    def train(self, env, batch_sz=32, updates=1000):
        ...
        for update in range(updates):
            for step in range(batch_sz):
                ...
                actions[step], values[step] = self.model.action_value(next_obs[None, :]) # from (4,) it becomes (1,4)
                ...
                if dones[step]:
                    ...
                    print("\rEpisode: {:d}  Reward: {:.2f}     ".format(
                        len(ep_rews) - 1, ep_rews[-2]), end="") # This produces a persistent screen mssg that updates only the Episode number and Reward!!!
    ...
    def _logits_loss(self, acts_and_advs, logits):
        ...
        policy_loss = weighted_sparse_ce(actions, logits, sample_weight=advantages) # Ai*log(pi)
